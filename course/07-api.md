...
> **本章目标**：会用 `curl` 调用只读状态接口与相机取图，正确调用受安全门控保护的运动与导航接口，并看懂每一种拒绝的含义。
> **涉及文件**：`src/xr_arm_console/xr_arm_console/app.py`（全部接口实现）

控制台后端本身就是一套 HTTP API，前缀统一为 `http://<机器人IP>:8088`。约定：`GET` 接口全部只读无副作用；`POST` 接口凡涉及真实运动一律要求**运动三件套**（见下文）。第六章工作流的 `http` 标签单元，底层调用的就是这些接口。
...
**按需发布与超时**：相机订阅是请求时才拉起的，服务端会短暂等待首帧（约 3 秒上限）。数据源不在线时返回 `503 {"code":"camera_frame_timeout","message":"等待相机画面超时"}`——第六章图 6-4 的工作流失败就是同一个机制。**处理方式：重试一次，或换一个在线相机**（第六章图 6-5 换 left 后 0.8s 成功）。

成功响应带两个有用的响应头：`X-XR-Frame-Sequence`（帧序号）与 `X-XR-Frame-Age`（帧龄，秒）。相机话题映射（`src/xr_arm_console/xr_arm_console/peripherals.py:10`）：
...
成功返回 `202 {"ok": true, "message": "升降指令已下发，目标 1.245 m"}`。常见拒绝：

| 响应 | 含义 |
|---|---|
| `403 missing_intent` | 缺意图请求头（三件套第一道闸） |
| `409 需要确认升降运动` | 缺 `confirm:true` |
| `400 delta_m 必须在 [-0.3, 0.3] 米内` | 相对目标越界 |
| `409 目标高度超出允许范围` | 绝对目标超行程 |
| `409 导航任务进行中，禁止升降运动` | 导航/升降互斥 |

头部运动同样模式：头 `X-XR-Body-Intent: head-motion`，体 `{"pitch_deg": -10, "yaw_deg": 15, "mode": "relative", "confirm": true}`。
...
1. 用 `curl` 依次请求 `/api/health`、`/api/status`、`/api/lift/status`，对照第一章/第四章界面数值。
2. 复现第六章图 6-4 的现象：请求一次 head 相机（可能 503），立即重试或换 left——用 API 亲眼验证"按需发布 + 重试"。
3. 故意省略意图头调用一次 `POST /api/lift/move`，观察 `403 missing_intent` 响应——三件套第一道闸（不会产生任何运动）。

## 小结

- `GET` 全只读可放心轮询；运动/导航类 `POST` 一律三件套：产品门控 + 意图头 + `confirm:true`。
- 相机按需发布，首跑可能超时；重试或换在线相机；帧龄看 `X-XR-Frame-Age`。
- 升降相对模式 `delta_m` 上限 ±0.3 m；越界/超程/导航互斥都会被拒绝且原因明确。
- 所有可能运动的接口在代码层 fail-closed：异常输入只会得到 4xx，不会得到危险动作。
